...
class Page:
    def __init__(self, now, total=0, size=12, roll=11, **kwargs):
        """
        qs      URL查询字符串的字典
        total   数据列表总数
        size    每页显示多少条
    ...
    def limit(self, sql=False):
        if sql:
            return "limit %s, %s" % ((self.now - 1) * self.size, self.size)
        return self.size, (self.now - 1) * self.size
    ...
    def href(self, page):
        ...
        if not hasattr(self, 'url'):
            return '?page=%s' % page
        return self.url.split('page')[0].rstrip('/') + '/page/%s' % page
    ...
    def first(self):
        ...
        if self.now > self.rollMid and self.totalPage > self.roll:
            return '<li><a href="%s">1</a></li>' % (self.href(1))
        return ''
    ...
    def last(self):
        ...
        if (self.totalPage > self.roll
            ...
            return ('<li><a href="%s">%s</a></li>'
                        % (self.href(self.totalPage), self.totalPage))
        return ''
    ...
    def num(self):
        ...
        for i in range(loopNum):
            ...
            html += '<li %s><a href="%s">%s</a></li>' % (active, self.href(i), i)
